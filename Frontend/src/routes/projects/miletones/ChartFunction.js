...
const ChartFunction = (milestoneId) => {
  ...
  async function getChartData(projectId, milestoneId) {
    ...
    if (milestoneId === "overall") {
      // Fetch the total tasks for all the milestones
      response = await axios.get(`${process.env.REACT_APP_API_SERVER}/api/projects/${projectId}/chart_data?milestoneId=overall`)
    } else {
      // Fetch the tasks for the selected milestone
      response = await axios.get(`${process.env.REACT_APP_API_SERVER}/api/projects/${projectId}/chart_data?milestoneId=${milestoneId}`)
    }
    if (response.status === 200) {
      console.log("Response Success")
      return await response.data
